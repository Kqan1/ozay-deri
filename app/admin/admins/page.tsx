...
import { getAdmins, createAdmin, deleteAdmin } from "../users/actions";
    ...
    async function handleDeleteAdmin(id: string) {
        ...
        toast("Emin misiniz?", {
            ...
            action: {
                label: "Evet, Sil",
                onClick: async () => {
                    const res = await deleteAdmin(id, (session.user as any).id);
                    if (res.error) {
                        toast.error(res.error);
                    } else {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            <div>
                ...
                <p className="text-muted-foreground mt-1">Sisteme erişimi olan tüm yöneticileri buradan yönetebilirsiniz.</p>
            </div>
                        ...
                        <form onSubmit={handleCreateAdmin} className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <input 
                                    ...
                                    className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" 
                                />
                                <p className="text-xs text-muted-foreground">Kullanıcı giriş yaptıktan sonra şifresini değiştirebilir.</p>
                            </div>
                ...
                <Card className="lg:col-span-2">
                    ...
                    <CardContent className="p-0">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody>
                                {admins.map(admin => {
                                    const isMe = session?.user && (session.user as any).id === admin.id;
                                    return (
                                        <tr key={admin.id} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                                            <td className="px-6 py-4 font-medium flex items-center gap-2">
                                                ...
                                                {admin.username}
                                                {isMe && <span className="ml-2 px-2 py-0.5 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 text-[10px] rounded-full uppercase tracking-wide font-bold">Siz</span>}
                                            </td>
                                            <td className="px-6 py-4">{new Date(admin.createdAt).toLocaleDateString('tr-TR')}</td>
                                ...
                                {admins.length === 0 && (
                                    <tr>
                                        <td colSpan={4} className="px-6 py-8 text-center text-muted-foreground">Kayıtlı yönetici bulunamadı.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
