...
export default function AdminDashboardPage() {
  ...
  const total =
    stats.productsCount + stats.categoriesCount + stats.fieldsCount || 1;
  const prodHeight = animateGraph
    ? `${Math.max((stats.productsCount / total) * 100, 5)}%`
    : "0%";
  const catHeight = animateGraph
    ? `${Math.max((stats.categoriesCount / total) * 100, 5)}%`
    : "0%";
  const fieldsHeight = animateGraph
    ? `${Math.max((stats.fieldsCount / total) * 100, 5)}%`
    : "0%";
