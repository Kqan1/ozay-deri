...
import { getFieldDefinitions, getCategories } from "../actions";
    ...
    return (
        <div className="space-y-8">
            <div>
                ...
                <p className="text-muted-foreground mt-1">Ürünler için global ve kategoriye özel alanları (renk, beden, kumaş vb.) tanımlayın.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-1 border rounded-lg p-6 bg-card">
                    <h2 className="text-xl font-semibold mb-4">{editFieldId ? "Alanı Düzenle" : "Yeni Alan Ekle"}</h2>
                    <form onSubmit={handleSaveField} className="space-y-4">
                        <div className="space-y-2">
                            <label className="text-sm font-medium">Alan Adı (Örn: Renk, Kumaş, Fiyat, Açıklama)</label>
                            <input 
                                value={name} 
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium">Kapsam (Nerede Geçerli?)</label>
                            <select 
                                value={isGlobal} 
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium">Kategori Seçimi (Eğer Belirli İse)</label>
                            <select 
                                value={categoryId} 
                        ...
                        <div className="space-y-2">
                            ...
                            <label className="flex items-center space-x-2 cursor-pointer">
                                <input 
                                    ...
                                    className="rounded border-gray-300 cursor-pointer" 
                                />
                                <span className="text-sm font-medium">Aranabilir (Kelime aramasında geçerli)</span>
                            </label>
                        </div>

                        <div className="flex gap-2">
                            <Button type="submit" className="w-full cursor-pointer">{editFieldId ? "Güncelle" : "Alanı Oluştur"}</Button>
                            {editFieldId && (
                                <Button 
                                    type="button" 
                                    variant="outline" 
                ...
                <div className="md:col-span-2 border rounded-lg p-0 overflow-hidden bg-card">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody>
                            {fields.map(f => (
                                <tr key={f.id} className="border-b last:border-0 hover:bg-muted/50">
                                    ...
                                    <td className="px-6 py-4 space-x-2 flex flex-wrap gap-2">
                                        ...
                                        {f.isSortable && <span className="px-2 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400 rounded text-xs">Sıralanabilir</span>}
                                        {f.isSearchable && <span className="px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 rounded text-xs">Aranabilir</span>}
                                    </td>
                            ...
                            {fields.length === 0 && (
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center text-muted-foreground">Henüz tanımlanmış bir alan bulunmuyor.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
