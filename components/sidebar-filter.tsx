...
export default function SidebarFilter({ filterableFields, categories = [] }: { filterableFields: FilterOption[], categories?: any[] }) {
  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div className="flex items-center justify-between border-b border-white/10 pb-4">
        ...
        {hasAnyFilter && (
          <button onClick={handleClear} className="text-xs text-rose-400 flex items-center hover:text-rose-300 transition-colors">
            Temizle
          </button>
        )}
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 max-h-48 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/10">
            {categories.filter(c => !c.parentId).map((cat) => {
              ...
              return (
                <label key={cat.id} className="flex items-center space-x-3 cursor-pointer group select-none">
                  <div className={`w-4 h-4 flex items-center justify-center rounded border transition-colors ${isChecked ? 'bg-indigo-600 border-indigo-600' : 'border-neutral-500 group-hover:border-neutral-400'}`}>
                  ...
                  />
                  <span className={`text-sm transition-colors ${isChecked ? 'text-white font-medium' : 'text-neutral-400 group-hover:text-neutral-300'}`}>
                    {cat.name}
                  </span>
                </label>
              );
      ...
      {filterableFields.map((field) => {
        ...
        const activeValues = searchParams.get(field.name) ? searchParams.get(field.name)!.split(",") : [];
          ...
          <div key={field.id} className="space-y-4">
            ...
            <div className="space-y-3">
              {field.options.map((opt) => {
                ...
                return (
                  <label key={opt} className="flex items-center space-x-3 cursor-pointer group select-none">
                    <div className={`w-4 h-4 flex items-center justify-center rounded border transition-colors ${isChecked ? 'bg-indigo-600 border-indigo-600' : 'border-neutral-500 group-hover:border-neutral-400'}`}>
                       {isChecked && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                    </div>
                    <input 
